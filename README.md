...
| `GATEWAY_AUTH_ENABLED` | `false` | Decouple gateway auth from provider credentials. When `false`, the bearer token is passed straight through to the provider (the default). When `true`, the bearer is the caller's **gateway key**: the proxy authenticates it against `GATEWAY_AUTH_TENANTS` and resolves the real provider key from `GATEWAY_SECRETS`, so callers never hold provider keys. Misconfiguration fails startup. |
| `GATEWAY_AUTH_TENANTS` | _(unset)_ | JSON tenant table, required when gateway auth is enabled. Keyed by the **SHA-256 hex digest** of each gateway key (raw keys are never stored), mapping to `{"tenant": "<id>", "providers": {"<provider>": "<secret-ref>"}}`. Example: `{"<sha256(gw-key)>":{"tenant":"acme","providers":{"openai":"acme-openai"}}}`. An unknown key gets `401`; a known key with no secret for the request's provider gets `403`. `vertexai` needs no secret — the tenant is authenticated and Vertex authenticates via ADC. |
| `GATEWAY_SECRETS` | _(unset)_ | Feeds the in-memory secret source: a comma-separated `ref=value` list resolving the references in `GATEWAY_AUTH_TENANTS` to provider keys, e.g. `acme-openai=sk-...`. The first `=` separates ref from value (values may contain `=`). A referenced ref with no value yields `500`. This static source is a stand-in for a future Google Secret Manager backend behind the same seam. |
